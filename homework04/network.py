...
def process_friends(users_ids: List[Any], vertices: List[str] = None) -> Tuple[List[dict], List[str]]:
        ...
        for friend_a in user_a['user_friends']:
            for friend_b in user_b['user_friends']:
                ...
                if friend_a['name'] == friend_b['name'] and friend_a['id'] != friend_b['id']:
                    friend_a['name'] = resolve_duplicate(
                        friend_a['name'], [friend_b])
    ...
    for user in users:
        while True:
            ...
            # Add him to friend list
                ...
                vertices.append(user['user_name'])
                continue
            break
...
def resolve_duplicate(name: str, user_friends: List[dict], recurr_lvl: int = 0) -> str:
    """ 
...
def create_vertices(users: List[dict]) -> List[str]:
...
def connect_mutuals(users: List[dict], vertices: List[str]) -> Tuple[list, list]:
    ...
    for user_a, user_b in itertools.combinations(users, 2):
        ...
        for friend_a in user_a['user_friends']:
            for friend_b in user_b['user_friends']:
                ...
                edges.append(
                    (vertices.index(friend_a['name']), user_a['index']))
                edges.append(
                    (vertices.index(friend_b['name']), user_b['index']))
...
def get_network(users_ids: List[Any]) -> None:
...
def plot_graph(vertices: List[str], edges: List[tuple], mutual_indexes: List[tuple] = None, users: List[dict] = None) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "ids", type=str, help="Comma separated list of VK users ids or screen names")
