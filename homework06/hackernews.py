...
def recommendations():
    ...
    train_titles = [title[0] for title in s.query(
        News.title).filter(News.label != None).all()]
    train_labels = [label[0] for label in s.query(
        News.label).filter(News.label != None).all()]
    ...
    classified_news = sorted(
        classified_news, key=lambda x: float(x['prob_sum']), reverse=True)
